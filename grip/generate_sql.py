...
for line in raw_data.split('\n'):
    ...
    if m_panch and current_taluka:
        ...
        if dept_name not in existing:
            ...
            escaped_dept = dept_name.replace("'", "''")
            sql = f"INSERT INTO departments (department_name, department_type, taluka_name, contact_email) VALUES ('{escaped_dept}', 'Panchayat', '{current_taluka}', '{email}');"
            sql_statements.append(sql)

with open('insert_panchayats.sql', 'w') as f:
    f.write('\n'.join(sql_statements))
print(f'Generated {len(sql_statements)} SQL INSERT statements in insert_panchayats.sql')
