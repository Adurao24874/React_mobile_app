...
export default function PWDDepartments({ departments, currentUser }: { departments: any[], currentUser: any }) {
    ...
    <div className="min-h-screen bg-slate-50 p-6 lg:p-10">
      ...
      <div className="mb-8 border-b border-slate-200 pb-6">
        ...
        <p className="text-slate-500 mt-1 font-medium">
          Logged in as <span className="text-blue-600 font-bold">{currentUser.name}</span> • {departments[0]?.department_name || "PWD"}
        </p>
      </div>
      ...
      <div className="space-y-8">
        {departments.map((dept: any) => (
          <div key={dept.id || dept.department_name} className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            
            {/* Department Title Area */}
            <div className="bg-slate-800 p-6 text-white flex justify-between items-center">
              <div>
                <h2 className="text-2xl font-bold">{dept.department_name}</h2>
                <p className="text-slate-300 text-sm mt-1">Taluka: {dept.taluka_name || "Unknown"}</p>
              </div>
              <div className="bg-slate-700 px-4 py-2 rounded-lg text-center">
                <p className="text-xs text-slate-300 uppercase tracking-wider font-bold">Active Staff</p>
                <p className="text-2xl font-black text-white">{dept.workers ? dept.workers.length : "..."}</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-3 divide-x divide-slate-100 border-b border-slate-100 bg-slate-50">
              <div className="p-4 text-center">
                ...
                <p className="text-2xl font-black text-slate-800">{dept.total_reports || 0}</p>
              </div>
              <div className="p-4 text-center">
                <p className="text-xs text-amber-500 font-bold uppercase tracking-wider">Pending</p>
                <p className="text-2xl font-black text-amber-600">{dept.pending_reports || 0}</p>
              </div>
              <div className="p-4 text-center">
                <p className="text-xs text-emerald-500 font-bold uppercase tracking-wider">Resolved</p>
                <p className="text-2xl font-black text-emerald-600">{dept.resolved_reports || 0}</p>
              </div>
            </div>

            {/* Workers List (Mocked/Omitted if not fetched in this specific query) */}
            <div className="p-6">
              ...
              {dept.workers && dept.workers.length > 0 ? (
                ...
                <p className="text-slate-500 italic">Personnel list is securely managed in the Field Workers roster.</p>
              )}
            </div>
