...
export default function PanchayatWorkerHome({ reports, workerName, deptName }: any) {
  const pendingCount = reports.filter((r: any) => r.status?.toLowerCase() !== 'resolved' && r.status?.toLowerCase() !== 'completed').length;
      ...
      <div className="bg-white p-4 lg:p-5 rounded-2xl shadow-sm border border-slate-100 flex flex-col">
          ...
          <div className="w-full h-[400px] lg:h-[600px] rounded-xl overflow-hidden border border-slate-200 relative z-0">
             <MapContainer center={[15.4909, 73.8278]} zoom={10} style={{ height: '100%', width: '100%' }} zoomControl={false} attributionControl={false}>
                ...
                <MapResizerAndFitter points={reports} />
                <DynamicMapLayers reports={reports} showReports={true} showSensors={false} showHeatmap={false} conditions={[]} getConditionColor={getConditionColor} />
            </MapContainer>
          </div>
      </div>
    </div>
