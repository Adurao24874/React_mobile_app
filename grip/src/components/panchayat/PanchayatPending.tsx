...
export default function PanchayatPending({ myAssignedTickets, onRefresh }: { myAssignedTickets: any[], onRefresh?: () => void }) {
  ...
  const reports = myAssignedTickets.filter((t: any) => {
      ...
      return (statusText !== "resolved" && statusText !== "completed") && !locallyResolvedIds.has(String(t.id));
  });
  ...
  const getRowStyle = (riskStatus?: string, isExpanded?: boolean) => {
    if (isExpanded) return "bg-emerald-50/30";
  ...
  const renderStatusBadge = (report: any) => {
    ...
    return (
      <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-emerald-50 text-emerald-700 shadow-sm whitespace-nowrap">
         ✓ On Track
      </span>
    );
  };
  ...
  const confirmResolution = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setResolutionMessage(result.message || 'Garbage cleared and verified!');
        ...
        <div className="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            ...
            <p className="text-slate-500 font-medium">
              Monitor your active garbage black spots and coordinate clearance.
            </p>
          </div>
        </div>
              ...
              <tbody className="divide-y divide-slate-100">
                {reports.map((report) => (
                  <React.Fragment key={report.id}>
                    ...
                    >
                      <td className="p-5">
                        <div className="flex items-center gap-3">
                          ...
                          {(report.display_type || report.issue_type || 'Unclassified').replace(/_/g, ' ')}
                          </span>
                        </div>
                      </td>
                      <td className="p-5">
                        <span className="font-medium text-slate-700 block flex items-center">
                      ...
                      <td className="p-5">
                        <div className="text-sm font-medium text-slate-900 flex items-center">
                          {report.worker_name || "Panchayat Team"}
                          <span className="text-[10px] bg-emerald-600 text-white px-1.5 py-0.5 rounded ml-2 uppercase">You</span>
                        </div>
                      </td>
                      <td className="p-5">
                      ...
                      <td className="p-5 text-center">
                        <button
                          className="text-emerald-600 hover:text-emerald-800 font-semibold text-sm focus:outline-none"
                          onClick={(e) => {
                            e.stopPropagation();
                            toggleRow(report.id);
                          }}
                    ...
                    {expandedRow === report.id && (
                      <tr>
                        <td colSpan={5} className="bg-slate-50 p-0 border-b-2 border-emerald-100">
                            ...
                            <div>
                              ...
                              <div className="bg-white p-3 sm:p-5 rounded-lg border border-slate-300 shadow-sm">
                                <p className="hidden sm:block text-sm text-slate-600 mb-4">
                                  This ticket is currently in the active field queue. The assigned Sanitation Worker will be required to upload a geotagged photo of the resolution at the site to close this ticket.
                                </p>
                                ...
                                {resolutionStatus === 'success' && resolvingId === null && locallyResolvedIds.has(String(report.id)) ? (
                                  ...
                                  <div className="p-3 sm:p-4 border border-emerald-100 rounded-xl bg-emerald-50/30 flex flex-col gap-3 sm:gap-4 sm:mt-4">
                                      ...
                                      <input 
                                          ...
                                          className="file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-emerald-100 file:text-emerald-700 hover:file:bg-emerald-200 cursor-pointer"
                                      />
                                      ...
                                      >
                                          {resolvingId === String(report.id) ? 'Verifying Location & Uploading...' : 'Upload MRF Photo & Resolve'}
                                      </button>
                  ...
                  <tr>
                    <td colSpan={5} className="p-12 text-center">
                      ...
                      <p className="text-slate-500">
                        You have no active waste issues assigned to you.
                      </p>
                    </td>
                  </tr>
                )}
